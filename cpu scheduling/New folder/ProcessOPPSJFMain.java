...
{ 
   ...
   private static int sumcpub=0,a[]=new int[20];
   private static float sumtat=0.0f,sumwt=0.0f;
   boolean status;

public static Scanner sc =new Scanner(System.in);
ProcessOPPSJF[] obj = new ProcessOPPSJF[20]; 

public void input(int n)
  {   
       int timer=0,k=0,j=0,mincb;
      for(int i=0;i<n;i++)
        {   
	   obj[i] = new ProcessOPPSJF();
           sc.nextLine();
   ...
   while(timer<sumcpub)
       {
          ...
          for(int i=0;i<n;i++)
            {
                if((obj[i].status==false)&&(obj[i].cpuburst<mincb))
                   {
...
public void ganttChart(int n)
  ...
  public class ProcessOPPSJFMain
     {
        public static void main(String arg[])
           {
  		 Scanner in = new Scanner(System.in);
   		 ProcessOPPSJF obj1 = new ProcessOPPSJF();
		 int n=0;
              while(true)
                {   
